...
ComputeBlurHor::ComputeBlurHor(ID3D11Device* device, HWND hwnd, int w, int h) : BaseShader(device, hwnd)
{
	sWidth = w;
	sHeight = h;
	initShader(L"computeBlurHor_cs.cso", NULL);
}
...
void ComputeBlurHor::initShader(const wchar_t* cfile, const wchar_t* blank)
{
	loadComputeShader(cfile);
	createOutputUAV();
}

void ComputeBlurHor::createOutputUAV()
{
...
void ComputeBlurHor::setShaderParameters(ID3D11DeviceContext* dc, ID3D11ShaderResourceView* texture1)
{
	dc->CSSetShaderResources(0, 1, &texture1);
	dc->CSSetUnorderedAccessViews(0, 1, &m_uavAccess, 0);
}

void ComputeBlurHor::unbind(ID3D11DeviceContext* dc)
{
